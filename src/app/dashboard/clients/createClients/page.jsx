...
export default function Page() {
  ...
  const {toast} = useToast()
		...
		const fetchData = async () => {
			try {
				const response = await axios.get(
					"http://localhost:3000/api/createClients",
				);
				setData(response.data.trainers);
				setNum(
					response.data.client.id_ciente
						? response.data.client.id_ciente + 1
	...
	const createClient = async () => {
		await axios
			.post("http://localhost:3000/api/createClients", {
				headers: {
				...
				<form className="align-middle text-center items-center p-12  bg-white w-[100%]">
					<div className=" md:flex md:flex-row flex flex-col">
						...
						<div className="md:w-[50%] w-[100%] flex flex-col">
							...
							/>
							<select
								value={info.entrenador}
								onChange={(e) =>
									setInfo({
										...info,
										entrenador: Number.parseInt(e.target.value),
									})
								}
								className="mb-6 w-[90%] h-[35px] px-4 shadow-md rounded-md border-none outline-none focus:outline-2 focus:outline-slate-500"
					...
					<div className="block gap-">
						...
						<button
							type="button"
							onClick={goBack}
							className="bg-red-700 px-16 py-3 rounded-md shadow-md text-white "
						>
							Cancelar
						</button>
					</div>
